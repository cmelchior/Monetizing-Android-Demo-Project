...
import dk.trifork.geeknight.billing.Security.VerifiedPurchase;
import dk.trifork.geeknight.billing.requests.BillingRequest;
import dk.trifork.geeknight.billing.requests.CheckBillingSupported;
import dk.trifork.geeknight.billing.requests.ConfirmNotifications;
import dk.trifork.geeknight.billing.requests.GetPurchaseInformation;
...
public class BillingService extends Service implements ServiceConnection {
    ...
    public void handleCommand(Intent intent, int startId) {
        ...
        if (Consts.DEBUG) {
            Logger.i(TAG, "handleCommand() action: " + action);
        }
    ...
    public boolean bindToMarketBillingService() {
        try {
            if (Consts.DEBUG) {
                Logger.i(TAG, "binding to Market billing service");
            }
            ...
            if (bindResult) {
                ...
                Logger.e(TAG, "Could not bind to service.");
            }
    ...
    public boolean checkBillingSupported() {
        return new CheckBillingSupported().runRequest(this, mService);
    }
    ...
    public boolean requestPurchase(String productId, String developerPayload) {
        return new RequestPurchase(productId, developerPayload).runRequest(this, mService);
    }
    ...
    public boolean restoreTransactions() {
        return new RestoreTransactions().runRequest(this, mService);
    }
    ...
    private boolean confirmNotifications(int startId, String[] notifyIds) {
        return new ConfirmNotifications(startId, notifyIds).runRequest(this, mService);
    }
    ...
    private boolean getPurchaseInformation(int startId, String[] notifyIds) {
        return new GetPurchaseInformation(startId, notifyIds).runRequest(this, mService);
    }
    ...
    private void checkResponseCode(long requestId, ResponseCode responseCode) {
        ...
        if (request != null) {
            if (Consts.DEBUG) {
                Logger.d(TAG, request.getClass().getSimpleName() + ": " + responseCode);
            }
    ...
    private void runPendingRequests() {
        ...
        while ((request = mPendingRequests.peek()) != null) {
            if (request.runIfConnected(mService)) {
                // Remove the request
        ...
        if (maxStartId >= 0) {
            if (Consts.DEBUG) {
                Logger.i(TAG, "stopping service, startId: " + maxStartId);
            }
            stopSelf(maxStartId);
        }
    }
    ...
    public void onServiceConnected(ComponentName name, IBinder service) {
        if (Consts.DEBUG) {
            Logger.d(TAG, "Billing service connected");
        }
    ...
    public void addRequest (BillingRequest request) {
    	mPendingRequests.add(request);
    }
    
    public void putSentRequest(long mRequestId, BillingRequest request) {
    	mSentRequests.put(mRequestId, request);
    }
}
